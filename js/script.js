        ...
        bgVideo:function(e){
            return this.each(function(e){
                ...
                var n={
                    ...
                    if(jQuery(window).width()>1024){r+='<video id="video'+e+'" preload="auto" autoplay="autoplay" loop="loop"';
                    if(n.videosound){}
                    else{r+=' muted="muted" '}
                    if(n.videoposter){
...
function sizeDownHeader(){
    ...
    if(headeroverlay){
        ...
        if(jQuery(document).scrollTop()>jQuery(window).height()-headerheight&&jQuery(".fixed-header").length>0){
            jQuery("#overlay-logo").fadeOut(0);jQuery("header").removeClass("header-overlay");
            ...
            if(jQuery(".header-shown").length>0){
                jQuery("header").animate({top:"-"+headerheight+"px"},200,function(){
                    jQuery("header").addClass("header-overlay");j
                    Query("header").animate({top:"0px"},600,"easeInOutQuart");
...
var headerheight=jQuery("header").height();// <header>
...
jQuery(window).scroll(function(){sizeDownHeader()});
...
jQuery(window).load(function(e){
    jQuery("body").on("click",'input[type="submit"]',function(){
        ...
        if(form_class=="checkform"){
            ...
            if(!e){
                jQuery("#form-note").fadeIn(200);return false
            }
            else{
